...
def add_project():
    ...
    if not customername or not techused or not projectid or not projectname or not elementchiefid or not flightdirectorid or not archived:
        return (
            jsonify(
                {"message": "You must fill in all fields to create a project"}), 400
        )
...
def add_war():
    if request.method == 'POST':
        ...
        if not projectid or not startdate or not enddate:
            return (
                jsonify(
                    {"message": "You must fill in all fields to create a war"}), 400
            )

        new_war = War(projectid=projectid,
...
def add_milestone():
    ...
    new_milestone = Milestones(
        date=date, description=description, status=status, projectid=projectid)
...
def add_employee():
    ...
    new_employee = Employees(jnumber=jnumber, firstname=firstname, lastname=lastname,
                             projectid=projectid, role=role, supervisorid=supervisorid)
